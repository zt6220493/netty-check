...
function ColumnResize(table) {
    ...
    var dragColumns; // first row columns, used for changing of width
    if(table.rows[0]){
    ...
    for (var i = 0; i < dragColumns.length; i++) {
    	...
    	var  isresizabled = $(dragColumns[i]).attr("isresizabled") ;
    	if (isresizabled !="false")
    	{
    		dragColumns[i].innerHTML = 
    			...
    			+"<div class='resizableDiv' style='height:100%;width:5px;margin-right:-10px;left:100%;top:0px;cursor:col-resize;z-index:10;float: right;'>&nbsp;</div>"
    			+"</span>";
        
            // BUGBUG: calculate real border width instead of 5px!!!
            dragColumns[i].lastChild.firstChild.onmousedown = this.startColumnDrag;
    	}
