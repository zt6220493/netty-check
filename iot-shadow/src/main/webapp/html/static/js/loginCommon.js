// 用户名和票据
var TicketUtil = {};
TicketUtil.ticketId = null;
TicketUtil.username = null;
TicketUtil.userVO = null;

TicketUtil.setTicketId = function(value) {
	TicketUtil.ticketId = value;
};
TicketUtil.setUsername = function(value) {
	TicketUtil.username = value;
};
TicketUtil.setUserVO = function(value) {
	TicketUtil.userVO = value;
};
...
TicketUtil.getTicketId = function() {
	return TicketUtil.ticketId;
}
TicketUtil.getUserName = function() {
	return TicketUtil.username;
};
TicketUtil.getUserVO = function() {
	return TicketUtil.userVO;
};
function loginInfo(data) {
	if(getCookie('ticketId') == null){
		setCookie('ticketId', data.data.ticketId);
	}
	TicketUtil.setTicketId(data.data.ticketId);
	TicketUtil.setUsername(data.data.username);
	TicketUtil.setUserVO(data.data.userVO);
}
function loadLoginInfo() {
	var url = prefix_url + "/eoc_ms/service/basic/enFindInfo";
	$.ajax( {
		type : "post",
		url : url,
		data : {
		...
		success : function(data) {
			if (data.isSucceed == 1) 
			{
				window.parent.location.href = '/eoc_ms/eoc/html/login/error.html';
			}
			else
			{
				loginInfo(data)
...
function addTicketToHref() {
	for ( var i = 0; i < $("a").length; i++) {
		var href = $("a:eq(" + i + ")").attr('href');
		if (href.indexOf(".html") > 0 && href.indexOf("?") > 0 && !href.indexOf("ticketId")>0) {
			// 已经有参数的有效链接
			$("a:eq(" + i + ")").attr('href',
					href + '&ticketId=' + TicketUtil.ticketId);
...
/**
 * 判断是否为ie8以上的版本
 * @return false 不是ie8以上的版本 包含IE8，true 是ie8以上的版本
 */
function isIE8() {
